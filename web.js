/* @zyf2e/monitor-web version ' + 2.1.29 */
'use strict';
...
__assign$3(__assign$3(__assign$3({}, WxAppEvents), WxPageEvents), ERRORTYPES);
var BREADCRUMBTYPES;
(function (BREADCRUMBTYPES) {
    ...
    BREADCRUMBTYPES["PAGE_ON_UNLOAD"] = "Page On Unload";
...
function extractErrorStack(ex, level) {
    ...
    return __assign$2(__assign$2({}, normal), { stack: stack });
}
...
var Options = (function () {
    ...
    Options.prototype.bindOptions = function (options) {
        if (options === void 0) { options = {}; }
        var beforeAppAjaxSend = options.beforeAppAjaxSend, enableTraceId = options.enableTraceId, filterXhrUrlRegExp = options.filterXhrUrlRegExp, traceIdFieldName = options.traceIdFieldName, throttleDelayTime = options.throttleDelayTime, includeHttpUrlTraceIdRegExp = options.includeHttpUrlTraceIdRegExp, appOnLaunch = options.appOnLaunch, appOnShow = options.appOnShow, appOnHide = options.appOnHide, pageOnUnload = options.pageOnUnload, pageOnShow = options.pageOnShow, pageOnHide = options.pageOnHide, onPageNotFound = options.onPageNotFound, onShareAppMessage = options.onShareAppMessage, onShareTimeline = options.onShareTimeline, onTabItemTap = options.onTabItemTap, wxNavigateToMiniProgram = options.wxNavigateToMiniProgram, triggerWxEvent = options.triggerWxEvent, maxDuplicateCount = options.maxDuplicateCount;
            ...
            return __generator(this, function (_a) {
                requestFun = function () {
                    ...
                    if (typeof _this.configReportWxRequest === 'function') {
                        ...
                        requestOptions = __assign$1(__assign$1({}, requestOptions), params);
                    }
                    requestOptions = __assign$1(__assign$1({}, requestOptions), { data: JSON.stringify(data), url: url });
...
function log(_a) {
    ...
    var error = __assign$1({ type: ERRORTYPES.LOG_ERROR, level: level, message: unknownToString(message), name: 'MITO.log', customTag: unknownToString(tag), time: getTimestamp(), url: isWxMiniEnv ? getCurrentRoute() : getLocationHref() }, errorInfo);
    breadcrumb.push({
...
function fetchReplace() {
    ...
    replaceOld(_global, EVENTTYPES.FETCH, function (originalFetch) {
        return function (url, config) {
            ...
            config = __assign$4(__assign$4({}, config), { headers: headers });
            return originalFetch.apply(_global, [url, config]).then(function (res) {
                var tempRes = res.clone();
                var eTime = getTimestamp();
                handlerData = __assign$4(__assign$4({}, handlerData), { elapsedTime: eTime - sTime, status: tempRes.status, time: sTime });
                ...
                if (isFilterHttpUrl(url))
                    return;
                handlerData = __assign$4(__assign$4({}, handlerData), { elapsedTime: eTime - sTime, status: 0, time: sTime });
                triggerHandlers(EVENTTYPES.FETCH, handlerData);
                throw err;
            });
        };
